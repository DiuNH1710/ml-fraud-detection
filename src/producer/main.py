...
class TransactionProducer():
    ...
    def validate_transaction(self, transaction: Dict[str, Any]) -> bool:
        try:
            ...
            logger.error(f'Invalid transactiopn : {e.message}')
    ...
    def generate_transaction(self) -> Optional[Dict[str, Any]]:
        ...
        if not is_fraud and amount < 2.0:
            # simulate rapid small transaction
        ...
        # merchant collusion
        if not is_fraud and merchant in self.high_rish_merchants:
        ...
        # ensure that final fraud rate is betwwen 1-2%
    ...
    def run_continuous_production(self, interval: float = 0.0):
        ...
        self.running = True
        logger.info("Starting produce for topic %s...", self.topic)
    ...
    def shutdown(self, signum=None, frame=None):
        if self.running:
            logger.info('Initialing shutdown ...')
            self.running = False
